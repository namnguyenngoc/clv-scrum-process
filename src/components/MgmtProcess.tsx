...
// 
  ...
  const clickupGetTask = async () => {
    if(clickID){
      ...
      .then(async function (response) {
        ...
        const subjectTask = await createSubjectTask(data);
        data.subjectTask = subjectTask;
  ...
  const createSubjectTask = async (task) => {
    ...
    let __task = await callAPIClickup(url,body);
  ...
  useEffect(()=>{
    ...
    // let tokenClipboard = navigator.clipboard.readText();
    // if(tokenClipboard) {
    ...
    <div className="grid grid-flow-row gap-3 sweet-loading">
        ...
        <div className="grid grid-flow-col gap-1">
            ...
            <div>
              <Button 
                ...
                onClick={() => {navigator.clipboard.writeText(clickTaskInfo.description); setCopyStatus("Copied")}}
                >
                <div className="grid grid-flow-col gap-1">
                  <svg
      ...
      <div className="grid grid-flow-row">
        <div className="grid grid-flow-col gap-1 pb-1">
          <div className="grid grid-flow-col gap-1">
            ...
            <div>
              <Button 
                disabled={!clickTaskInfo || clickTaskInfo.description == "" ? true : false }
                onClick={() => {navigator.clipboard.writeText(clickTaskInfo.description); setCopyDescription("Copied")}}
                >
                <div className="grid grid-flow-col gap-1">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
        ...
        <textarea
          id="description"
          rows="10"
          value={clickTaskInfo ? clickTaskInfo.description : ""}
          onClick={handleKeyDownClickup}
          className="col-span-2 border border-gray-500 px-4 py-2 rounded-lg"
        />
      </div>
